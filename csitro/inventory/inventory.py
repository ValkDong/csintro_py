...
class Inventory:
    def __init__(self, app, textures) -> None:
        ...
        self.slots[0] = WeaponItem('short_sword', 1)
    ...
    def add_item(self, item):
        ...
        if target_slot < len(self.slots):
            ...
            self.slots[first_available_slot].quantity += items[item.name].quantity
    def update(self):
        if EventHandler.keydown(pygame.K_RIGHT): #moving right in slot

            if self.active_slot < len(self.slots) -1 :
                self.active_slot += 1
            print(f'Active slot:{self.active_slot}')
        if EventHandler.keydown(pygame.K_LEFT): #moving left in slot
    ...
    def draw(self):
        ...
        for i in range(len(self.slots)):
            if i == self.active_slot:
                pygame.draw.rect(self.screen, 'yellow', pygame.Rect(i*(TILESIZE*2), 0, TILESIZE*2, TILESIZE*2))
