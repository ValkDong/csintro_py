...
class BlockItem(Item):
    ...
    def use(self, player, position: tuple):
        if self.quantity > 0:
            items[self.name].use_type([player.group_list[group] for group in items[self.name].groups],pl
            self.quantity -= 1
            if self.quantity <= 0:self.name = "default"
        else:
            self.name = "default"

class ShortSwordItem(Item):
def __init__(self, name: str = "default", quantity: int = ) -> None:
    super().__init__(name, quantity)
def use(self, player, position: tuple):
    print('using sword')
...
class ItemData:
    def __init__(self, name: str, quantity: int = 1, groups: list[str] = ['sprites', 'block group'l, use
        self.name = name
        self.quantity = quantity
        self.groups = groups
